  ...
  return (
    <div className="grid h-full min-h-[320px] grid-cols-1 pt-11 sm:grid-cols-[1.5fr_.78fr]">
      <div className="product-grid relative flex items-center justify-center p-4 sm:p-6">
        ...
        <motion.div animate={{ opacity: show ? 1 : 0, y: show ? 0 : 16 }} className="absolute inset-x-5 bottom-4 rounded-xl border border-[#a9c4ff] bg-white p-3 shadow-[0_15px_35px_rgba(39,86,190,.13)]" transition={{ delay: 0.48, duration: 0.6, ease }}>
          <div className="flex items-center justify-between"><span className="flex items-center gap-2 text-xs font-medium"><i className="grid size-6 place-items-center rounded-full bg-[#246bfd] text-[11px] text-white">✦</i> Answer</span><span className="font-mono text-[8px] text-[#166c52]">94% confidence</span></div>
      ...
      <div className="hidden border-l border-[#deddd8] bg-[#faf9f6] p-3 pt-5 sm:block">
        ...
        {["Agreement.pdf", "Renewal schedule"].map((item, index) => (
          <motion.div animate={{ opacity: show ? 1 : 0, x: show ? 0 : 8 }} className={cn("mt-3 rounded-lg border p-3", index === 0 ? "border-[#a9c4ff] bg-white shadow-sm" : "border-[#deddd8] bg-[#fdfcf9]")} key={item} transition={{ delay: 0.16 + index * 0.12, duration: 0.5, ease }}>
    ...
    <div className="product-grid flex h-full min-h-[320px] flex-col justify-center px-5 pb-5 pt-16">
      <div className="grid grid-cols-[.82fr_auto_1.18fr] items-center gap-3">
        <motion.div animate={{ opacity: show ? 1 : 0, x: show ? 0 : -10 }} className="rounded-xl border border-[#deddd8] bg-[#fdfcf9] p-4 shadow-sm" transition={{ duration: 0.55, ease }}>
          ...
          <span className="mt-4 inline-flex rounded-full bg-[#eef3ff] px-2 py-1 font-mono text-[8px] text-[#2456c4]">80k records</span>
        </motion.div>
        <motion.div animate={{ opacity: show ? 1 : 0, scaleX: show ? 1 : 0 }} className="h-px w-8 origin-left bg-[#8cadff]" transition={{ delay: 0.28, duration: 0.45, ease }} />
        <motion.div animate={{ opacity: show ? 1 : 0, x: show ? 0 : 10 }} className="rounded-xl border border-[#a9c4ff] bg-white p-4 shadow-[0_12px_30px_rgba(39,86,190,.1)]" transition={{ delay: 0.35, duration: 0.55, ease }}>
          <div className="flex items-center justify-between"><p className="font-mono text-[8px] uppercase tracking-[0.15em] text-[#2456c4]">Induced hierarchy</p><span className="rounded-full bg-[#fff3c8] px-2 py-1 font-mono text-[8px] text-[#8b5c00]">F1 · 0.74</span></div>
          <div className="mt-4 rounded-lg border border-[#d9e4ff] bg-[#eef3ff] px-3 py-2 text-[10px] font-medium">Customer signals</div>
          <div className="ml-5 h-3 border-l border-[#8cadff]" />
      ...
      <motion.div animate={{ opacity: show ? 1 : 0, y: show ? 0 : 8 }} className="mt-4 flex items-center justify-between rounded-lg border border-[#deddd8] bg-[#fdfcf9]/90 px-3 py-2 font-mono text-[8px] text-[#6b707c]" transition={{ delay: 0.85, duration: 0.5, ease }}><span>BERTopic · UMAP · HDBSCAN</span><span className="text-[#2456c4]">Structure validated</span></motion.div>
    </div>
  );
}
    ...
    <div className="grid h-full min-h-[320px] grid-cols-[1.1fr_.9fr] gap-3 px-4 pb-4 pt-14">
      <div className="flex flex-col justify-center rounded-xl border border-[#deddd8] bg-[#fdfcf9] p-4 shadow-sm">
        ...
        <div className="mt-5 flex h-36 items-end gap-2 border-b border-l border-[#deddd8] px-2 pb-1">
          {[40, 56, 48, 70, 64, 91].map((height, index) => <motion.div animate={{ scaleY: show ? 1 : 0.05 }} className={cn("flex-1 origin-bottom rounded-t", index === 5 ? "bg-[#246bfd]" : "bg-[#b8caff]")} key={height} style={{ height: `${height}%` }} transition={{ delay: 0.08 * index, duration: 0.62, ease }} />)}
        </div>
      ...
      <div className="flex flex-col justify-center gap-2">
        {[["Semantic quality", ".83"], ["Boundary ambiguity", ".71"], ["Hierarchy utility", ".64"]].map(([label, score], index) => (
          <motion.div animate={{ opacity: show ? 1 : 0, x: show ? 0 : 8 }} className="rounded-lg border border-[#deddd8] bg-[#fdfcf9] p-3" key={label} transition={{ delay: 0.25 + index * 0.1, duration: 0.48, ease }}>
            <div className="flex items-center justify-between"><span className="text-[9px] font-medium">{label}</span><span className="font-mono text-[8px] text-[#2456c4]">{score}</span></div>
            <div className="mt-2 h-1 overflow-hidden rounded bg-[#e8e9ec]"><motion.div animate={{ scaleX: show ? 1 : 0 }} className="h-full origin-left rounded bg-[#8cadff]" style={{ width: `${83 - index * 11}%` }} transition={{ delay: 0.45 + index * 0.1, duration: 0.55, ease }} /></div>
          </motion.div>
        ))}
    ...
    <div className="product-grid flex h-full min-h-[320px] items-center justify-center px-4 pb-4 pt-14">
      <motion.div animate={{ opacity: show ? 1 : 0, y: show ? 0 : 12 }} className="relative h-[222px] w-[118px] rounded-[1.65rem] border border-[#cfd2d8] bg-[#fdfcf9] p-2 shadow-[0_16px_35px_rgba(27,32,45,.14)]" transition={{ duration: 0.6, ease }}>
        ...
        <div className="flex h-full flex-col items-center justify-center rounded-[1.2rem] border border-[#e1e2e4] bg-white px-3 text-center">
          <motion.div animate={{ scale: show ? 1 : 0.8 }} className="grid size-12 place-items-center rounded-full border border-[#a9c4ff] bg-[#eef3ff] text-lg text-[#2456c4]" transition={{ delay: 0.18, duration: 0.48, ease }}>♻</motion.div>
          <p className="mt-4 text-xs font-medium">Recyclable</p><p className="mt-1 font-mono text-[8px] text-[#727884]">82% confidence</p>
          <motion.div animate={{ opacity: show ? 1 : 0, y: show ? 0 : 5 }} className="mt-4 w-full rounded-full bg-[#246bfd] px-2 py-1.5 font-mono text-[7px] font-semibold text-white" transition={{ delay: 0.48, duration: 0.45, ease }}>SORT LOCALLY</motion.div>
        </div>
      </motion.div>
