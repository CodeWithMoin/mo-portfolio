...
`functions/api/contact.ts` sends the closing panel's form through [Resend](https://resend.com). Set `RESEND_API_KEY` (secret). Optional: `CONTACT_TO` (default `hello@moinuddin.app`) and `CONTACT_FROM` (default Resend's `onboarding@resend.dev`, which only delivers to the Resend account's own address — verify `moinuddin.app` in Resend and set e.g. `Portfolio <contact@moinuddin.app>`). Guardrails: same-origin + JSON, 8 KB cap, field validation, honeypot, minimum time-on-page, 3 messages per IP per day (shares the `ASK_LIMITS` KV). Replies go straight to the sender via `reply_to`. Without a key the form tells visitors to email instead.
